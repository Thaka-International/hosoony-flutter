...
{
    /**
     * Show evaluation form for a session.
     */
    public function showEvaluationForm(Session $session)
    {
        $teacher = Auth::user();
        
        // Check if teacher is assigned to this session
        if ($session->teacher_id !== $teacher->id) {
            abort(403, 'غير مخول لك تقييم هذه الجلسة');
        }

        // Get students in the class
        $students = $session->class->students()->where('role', 'student')->get();
        
        // Get existing evaluations for this session
        $existingEvaluations = PerformanceEvaluation::where('session_id', $session->id)
            ->get()
            ->keyBy('student_id');

        return view('pwa.teacher.evaluation-form', compact(
            'session', 
            'students', 
            'existingEvaluations'
        ));
    ...
    public function getEvaluation(PerformanceEvaluation $evaluation)
    {
        ...
        // Check if teacher can view this evaluation
        if ($evaluation->teacher_id !== $teacher->id && !$teacher->hasRole('admin')) {
            abort(403, 'غير مخول لك عرض هذا التقييم');
        }
    ...
    public function getStudentPerformanceHistory(User $student)
    {
        ...
        // Check if teacher can view this student's performance
        if (!$teacher->hasRole('admin') && !$teacher->classes()->whereHas('students', function($query) use ($student) {
            $query->where('user_id', $student->id);
        })->exists()) {
            abort(403, 'غير مخول لك عرض أداء هذه الطالبة');
        }
    ...
    public function getClassPerformanceSummary(ClassModel $class)
    {
        ...
        if (!$teacher->classes()->where('classes.id', $class->id)->exists() && !$teacher->hasRole('admin')) {
            abort(403, 'غير مخول لك عرض أداء هذا الفصل');
        }
    ...
    private function calculateSemesterAverage(User $student): float
    {
        ...
        return PerformanceEvaluation::where('student_id', $student->id)
            ->completed()
            ->inDateRange($startOfSemester, $endOfSemester)
            ->avg('total_score') ?? 0;
    }
    ...
    public function bulkEvaluate(Request $request, Session $session)
    {
        ...
        DB::transaction(function () use ($request, $session, $teacher, &$evaluations) {
